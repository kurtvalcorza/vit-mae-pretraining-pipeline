#!/usr/bin/env python
"""Download and verify base-model weights for the ViT-MAE pre-training pipeline.

Downloads and verifies the pinned Hugging Face ViT-MAE checkpoint
...
Usage:
    ...
    # Download default model (vit-mae-base) into weights/vit-mae-base/:
...
from vit_mae_pipeline.config import (  # noqa: E402
    DEFAULT_MODEL_KEY,
    MODEL_FILENAME,
    MODEL_ID,
    MODEL_REVISION,
...
def fetch_model(
    ...
    """Download and verify the ViT-MAE model checkpoint."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download and verify base-model weights for the ViT-MAE pipeline."
    )
    parser.add_argument(
        "--dest",
        type=Path,
